...
class Solution:
    def predictPartyVictory(self, senate):
        ...
        while radiant_queue and dire_queue:
            radiant_idx = radiant_queue.popleft()
            dire_idx = dire_queue.popleft()

            if radiant_idx < dire_idx:
                # Radiant голосует раньше, банит Dire
                # Radiant получает право голоса в следующем раунде
                radiant_queue.append(radiant_idx + n)
            else:
                # Dire голосует раньше, банит Radiant
                # Dire получает право голоса в следующем раунде
...
solver = Solution()
print (solver.predictPartyVictory("RDD"))
